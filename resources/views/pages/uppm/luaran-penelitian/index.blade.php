...
@section('content')
    <div class="flex-1 overflow-x-auto px-2">
        <div class="space-y-2 mb-2">
            <a href="banner">
                <h1 class="font-bold text-2xl">Luaran Penelitian</h1>
            </a>
            ...
            <span role="button" onclick="copyLinkAPI('/api/luaranpenelitianuppm')"
                class="inline-block text-sm rounded-lg text-sky-600 bg-slate-200 px-5 py-2"><i class="fa-solid fa-link"></i>
                <span id="linkAPI">/api/luaranpenelitianuppm</span></span>
        </div>
        <form action="{{ route('luaranpenelitianuppm.store') }}" class="flex flex-col md:flex-row md:items-start gap-4 py-3"
            ...
            <div class="flex-1 space-y-1">
                <div>
                    ...
                    <small class="mt-2 text-xs text-red-500">
                        {{ $errors->first('publication') }}</small>
                </div>
                <div>
                    <input type="text" name="indexjurnal"
                    ...
                    @forelse($luaran_uppms as $number => $uppm)
                        <tr class="bg-white border-b transition ease-in-out duration-200 hover:bg-gray-50">
                            ...
                            <td class="space-y-2 px-6 py-4">
                                <!-- Toggle -->
                                <form action="{{ route('luaranpenelitianuppm.update', $uppm->id) }}" method="POST" class="inline">
                                    @csrf
                                    @method('PATCH')
                                    ...
                                    <input type="hidden" name="publication" value="{{ $uppm->publication }}">
                                    <input type="hidden" name="indexjurnal" value="{{ $uppm->indexjurnal }}">
                                ...
                                <button
                                    class="w-full md:w-auto block md:inline text-center bg-amber-400 px-2 py-1 text-sm rounded text-white"><a
                                        href="{{ route('luaranpenelitianuppm.edit', $uppm->id) }}"><i
                                        ...
                                        <div class="relative bg-white rounded-lg shadow">
                                            ...
                                            <div class="flex flex-col p-6 text-center">
                                                ...
                                                <div class="flex justify-center gap-2">
                                                    <form action="{{ route('luaranpenelitianuppm.destroy', $uppm->id) }}"
                                                        method="POST">
                                                        @csrf
                                                        @method('DELETE')
                                                        <button role="button"
