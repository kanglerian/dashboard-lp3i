...
@section('content')
    <div class="flex-1 overflow-x-auto px-2">
        <div class="space-y-2">
            <a href="{{ route('agenda.index') }}" class="inline-block text-sm bg-slate-100 text-slate-900 px-4 py-2 rounded-lg mb-2"><span><i class="fa-solid fa-circle-chevron-left"></i></span> Kembali</a>
        ...
        <div class="flex flex-col md:flex-row gap-5 mt-5">
            <div class="w-full md:w-2/4 order-2 md:order-none">
                <form action="{{ route('agenda.update', $agenda->id) }}" class="flex flex-col gap-2" method="POST"
                    ...
                    <div class="flex-1">
                        <input type="date" name="date" value="{{ $agenda->date }}"
                    ...
                    <div class="flex-2">
                        <select name="status"
                            ...
                            <option value="{{ $agenda->status == 1 ? 0 : 1 }}">{{ $agenda->status == 1 ? 'Tidak aktif' : 'Aktif' }}
                            </option>
                        </select>
                        <small class="mt-2 text-xs text-red-500">
            ...
            <div class="w-full md:w-2/4 order-1 md:order-none">
                <img src="{{ asset($agenda->image) }}" alt="{{ $agenda->title }}"
