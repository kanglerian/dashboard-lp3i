...
@section('content')
    <div class="flex-1 overflow-x-auto px-2">
        <div class="space-y-2 mb-2">
            <a href="banner">
                <h1 class="font-bold text-2xl">Fasilitas</h1>
            </a>
            ...
            <span role="button" onclick="copyLinkAPI('/api/facilities')"
                class="inline-block text-sm rounded-lg text-sky-600 bg-slate-200 px-5 py-2"><i class="fa-solid fa-link"></i>
                <span id="linkAPI">/api/facilities</span></span>
        </div>
        <form action="{{ route('facility.store') }}" class="flex flex-col md:flex-row md:items-start gap-4 py-3"
        ...
        <div class="relative overflow-x-auto h-screen border border-gray-300 rounded-lg">
            <table class="w-full text-sm text-left text-gray-500">
                ...
                <tbody>
                    @forelse($facilities as $number => $facility)
                        <tr class="bg-white border-b transition ease-in-out duration-200 hover:bg-gray-50">
                            <td class="px-6 py-4">
                                {{ $number + 1 }}
                            </td>
                            <th scope="row" class="px-6 py-4">
                                <img loading="lazy" src="{{ asset($facility->image) }}" class="w-32 rounded">
                            </th>
                            ...
                            <td colspan="2" class="space-y-2 px-6 py-4">
                                <!-- Toggle -->
                                <form action="{{ route('facility.update', $facility->id) }}" method="POST" class="inline">
                                    @csrf
                                    @method('PATCH')
                                    <input type="hidden" name="title" value="{{ $facility->title }}">
                                    <input type="hidden" name="status" value="{{ $facility->status == 1 ? 0 : 1 }}">
                                    <button role="button" type="submit"
                                        class="w-full md:w-auto block md:inline text-center text-white px-2 py-1 text-sm rounded {{ $facility->status == 1 ? 'bg-blue-500' : 'bg-red-500' }}">{!! $facility->status == 1
                                ...
                                <button
                                    class="w-full md:w-auto block md:inline text-center bg-amber-400 px-2 py-1 text-sm rounded text-white"><a
                                        href="{{ route('facility.edit', $facility->id) }}"><i
                                            class="fa-regular fa-pen-to-square"></i></a></button>
                                <!-- Delete -->
                                <button role="button" data-modal-target="popup-modal{{ $facility->id }}"
                                    data-modal-toggle="popup-modal{{ $facility->id }}"
                                    class="w-full md:w-auto block md:inline text-center bg-red-600 px-2 py-1 text-sm rounded text-white"><i
                                        class="fa-solid fa-trash"></i></button>
                                <div id="popup-modal{{ $facility->id }}" tabindex="-1"
                                    ...
                                    <div class="relative w-full h-full max-w-md md:h-auto">
                                        <div class="relative bg-white rounded-lg shadow">
                                            <button type="button"
                                                ...
                                                data-modal-hide="popup-modal{{ $facility->id }}">
                                                <i class="fa-solid fa-xmark"></i>
                                            </button>
                                            <div class="flex flex-col p-6 text-center">
                                                ...
                                                <div class="flex justify-center gap-2">
                                                    ...
                                                    <button data-modal-hide="popup-modal{{ $facility->id }}"
                                                        type="button"
